...
- 이번 단계는 Campaign·Dungeon·Room에 집중합니다. 이전 버전의 캐릭터·몬스터·NPC·조우 자료와 테스트는 보존하며 새 생성 기능을 추가하지 않았습니다. 새 캠페인에서는 이들 생성 메뉴를 표시하지 않습니다.

## localStorage와 버전

저장 키: `morkborg-codex:v2`
...
1. v2 자료가 있으면 먼저 검증하고 복원합니다.
2. v2가 없고 v1이 있으면 생성 문구·직접 입력·시간·유효한 UUID를 유지해 v2로 옮깁니다.
3. 이전 원본 키를 삭제하거나 덮어쓰지 않습니다. 변환 전 JSON 문자열을 `morkborg-codex:pre-v2-backup`에도 보관한 뒤, 검증된 v2를 기록합니다. 백업 저장에 실패하면 v2 기록을 진행하지 않습니다.
4. v1이 없는 경우에만 해당 origin에 실제 존재하는 MÖRK BORG 이름의 키를 확인합니다. 룰북과 백업 키는 제외합니다. 지역·제목·방 배열·생성 필드가 확인되는 단일 던전 객체, `{dungeon: ...}`, `{currentDungeon: ...}`는 `Untitled Campaign`으로 감쌉니다. `generatedFields` 안의 문자열 및 `{value, source}`도 지원합니다.
5. 유효한 UUID는 유지하고, 없거나 숫자 인덱스인 ID는 최초 변환 시 UUID를 부여합니다. 이후에는 그 ID를 복원합니다. 원문의 공란에 새 생성 결과를 넣지 않습니다.
6. 알려지지 않은 버전·지역·끊어진 참조·지원하지 않는 내장 보관함은 부분적으로 추정 변환하지 않습니다. 원본은 유지됩니다. 실제 v1 키의 손상은 복구 화면을 표시하고, 다른 키의 판별 불가 데이터는 그 키에 그대로 둡니다.

`소개 및 출처 → 변환 전 저장 원본 내보내기`에서 백업을 받을 수 있습니다. 원문에 추가로 존재하던 미지원 속성도 변환 전 원본 문자열에 보존됩니다.

## Export / Import

Campaign JSON은 `{schemaVersion: 2, campaign: ...}`이며 모든 Dungeon·Room·노트·출처·미선택 후보를 포함합니다. 기존 v1 JSON도 읽을 수 있습니다.

가져오기 시 기존 Campaign 또는 그 안의 소유 개체 UUID와 하나라도 충돌하면 Campaign·Dungeon·Room·기존 연결 전체를 새 UUID로 복제합니다. 충돌이 없다면 원래 ID를 유지합니다. 기존 Campaign을 덮어쓰지 않으며, 가져온 Campaign의 Dungeon Library를 엽니다.
