"""Build the completion report using reviewed, deduplicated audit counts."""
from pathlib import Path
import json
ROOT=Path(__file__).resolve().parents[1]
DATA=ROOT/'data'
metrics=json.loads((DATA/'coverage.json').read_text())
groups=json.loads((DATA/'top-20.json').read_text())

def table(headers,rows):
    def esc(v):return str(v).replace('|','\\|').replace('\n',' ')
    return '\n'.join(['| '+' | '.join(headers)+' |','| '+' | '.join(['---']*len(headers))+' |']+['| '+' | '.join(esc(v) for v in row)+' |' for row in rows])

names={'core':'[Core Bare Bones](CORE-BARE-BONES.md)','core-full':'[Core Full Edition](CORE-FULL.md)','feretory':'[FERETORY](FERETORY.md)','heretic':'[HERETIC](HERETIC.md)','sd':'[Sölitary Defilement](SD.md)','depths':'[Sölitary Depths](DEPTHS.md)','reclvse':'[RECLVSE](RECLVSE.md)','mythic':'[Mythic GME 2e](MYTHIC.md)','aitc':'[Alöne in the Crowd](ALONE-IN-THE-CROWD.md)'}
statuses=list(metrics['classifications'])
body=f'''# Complete PDF Escape Audit — 최종 보고서
...
감사한 최종 애플리케이션 HEAD: `{metrics['head']}`. 원문 전수 검토와 넓은 브라우저 시나리오는 `a50d2425deb9a4e9c7e7abb0983dd4c34c68c116`에서 시작했다. 이후 실제 production diff, 설치된 데이터, 검색, 변경된 결과 표시를 재검사했다. [현재 HEAD 재검사](CURRENT-HEAD-RECHECK.md)가 이전 관찰 중 달라진 사항을 명시한다. 이번 완료 작업의 변경 범위는 감사 문서·데이터·감사용 도구다. 앱 수정과 배포는 수행하지 않았다.
...
{table(['분류','고유 필요','전체 1,268개 대비','플레이 관련 1,220개 대비'],[[s,metrics['classifications'][s],f"{metrics['classifications'][s]/1268*100:.2f}%",'제외' if s=='PDF_APPROPRIATE' else f"{metrics['classifications'][s]/1220*100:.2f}%"] for s in statuses])}
...
{table(['책','PDF 페이지','플레이 필요','Resolved','Indirect','Partial','Missing','PDF 적절','Source unavailable'],[[names[b['bookId']],b['pdfPages'],b['playRelevant'],*[b[s] for s in statuses]] for b in metrics['byBook']])}
...
{table(['워크플로','플레이 필요','Resolved','Indirect','Partial','Missing','PDF 적절','Source unavailable'],[[w['workflow'],w['playRelevant'],*[w[s] for s in statuses]] for w in metrics['byWorkflow']])}
...
{table(['순위','PDF 검색 상황','심각도','빈도 × 마찰','적용 범위'],[[g['rank'],g['title'],g['severity'],f"{g['frequency']} × {g['friction']} = {g['priority']}",g['scope']] for g in groups])}
...
'''
(ROOT/'FINAL-AUDIT-REPORT.md').write_text(body)
print('Rendered final audit report.')
