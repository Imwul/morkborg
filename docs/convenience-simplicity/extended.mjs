...
const root = 'outputs/convenience-simplicity/regression',
...
try {
  ...
  await click(
    page.getByRole('button', { name: 'QA FIRST CONTACT RUN', exact: true }),
  );
  ...
  await click(
    (await page
      .getByRole('button', { name: 'RUN · 고정 제외', exact: true })
      .isVisible())
      ? page.getByRole('button', { name: 'RUN · 고정 제외', exact: true })
      : page.getByRole('button', {
          name: 'QA FIRST CONTACT RUN',
          exact: true,
        }),
  );
  assert.equal(
  ...
  if (await page.getByRole('button', { name: '+ 만들기', exact: true }).count())
  ...
  await click(page.getByRole('button', { name: 'Last 비우기', exact: true }));
  await click(
    page.getByRole('button', { name: 'Scratch 비우기', exact: true }),
  );
