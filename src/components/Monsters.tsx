...
export function Monsters({
  ...
  function randomize() {
    const guardedConfirm: Confirm = (title, body, action) =>
      selected && hasManualEdits(selected)
  ...
  return (
    <div className="monster-workbench">
      ...
      <section
        className="monster-statblock-reading"
        aria-label="몬스터 핵심 결과"
      >
      ...
      <GenerationDisclosure
        values={{
          ...
          ...Object.fromEntries(
            selected.attacks.flatMap((attack, i) =>
              Object.entries(attack.fieldProvenance ?? {}).map(
                ([key, value]) => [`Attack ${i + 1} · ${key}`, value],
              ),
            ),
          ),
          ...Object.fromEntries(
            [
              ...
              item.provenance ? [[`Detail ${i + 1}`, item.provenance]] : [],
            ),
          ),
        }}
      />
      ...
      <details className="object-editor">
        ...
        <SourceDisclosure label="생성 규칙과 출처">
          {selected.generation?.system === 'epk' ? (
            <>
              <p>
                ...
                지역의 원문 주사위 표를 굴리고 해당 항목을 불러옵니다. 수치가
                없는 결과는 이름과 확인된 내용만 표시합니다.
              </p>
              <p>
                <SourceText text={selected.sources?.name} />
              </p>
              <p>
                Sölitary Depths의 일치하는 지역 항목에는 원문 PDF·쪽수 참조를
                함께 보관합니다. Galgenbeck은 Tveland를 참조하며, Grift는 Eat
                Prey Kill의 자체 지역 표를 사용합니다.
              </p>
            </>
          ) : isFeretory ? (
            <p>
              FERETORY · The Monster Approaches. 외형과 능력치는 같은 A/B/C
              결과를 사용합니다. 이름은 중립 표기이며, 공격명·독립
              행동·약점·전리품은 직접 작성합니다.
            </p>
          ) : (
            <p>
          ...
          {isFeretory && (
            <p>
              공격명과 피해를 분리해 기록합니다. FERETORY The Monster
              Approaches의 A/B/C 재굴림은 외형·능력치의 자동값을 함께 바꿉니다.
              HP 항목의 재굴림은 현재 피해 주사위만 다시 굴립니다.
            </p>
          )}
        </SourceDisclosure>
        {!rules.pack && (
        ...
        >
          <div className="monster-identity-grid">
            {scalar({ key: 'name', label: '몬스터 이름' }, undefined)}
          ...
          {isFeretory && (
            <div className="monster-linked-roll">
              <div>
                ...
                <p>
                  같은 3d12 → 외형·사기·피해·방어구 · HP는 피해 주사위 1회 ×2
                </p>
              </div>
              <Button
                className="btn small"
                ...
                <div className="character-item monster-attack" key={a.id}>
                  ...
                  {(['name', 'damage', 'description'] as const).map((key) => (
                    <Field
                      ...
                      provenance={a.fieldProvenance?.[key]}
                      onChange={(value, source) =>
                        edit((m) => {
