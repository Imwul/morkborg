  ...
  return (
    <section className="dungeon-crawl" aria-label="던전 크롤 작업 공간">
      <header className="crawl-overview">
        <div>
          <span className="eyebrow">ENTRANCE → ROOMS → CLIMAX</span>
          <h2>한 방씩, 아래로.</h2>
        </div>
      ...
      <details className="crawl-preparation">
        ...
        <p>
          Strong Hit마다 다음 특별한 방을 발견합니다. 네 번째 방이 절정입니다.
        </p>
        <ol>
          {(state
        ...
        <>
          ...
          {roll && (
            <output className={`crawl-outcome ${roll.outcome}`}>
              ...
              {roll.exhausted && (
                <small>특별한 방을 모두 발견하여 Strong → Weak</small>
              )}
          ...
          {state.phase === 'danger' && (
            <article className="crawl-step crawl-danger">
              ...
              <small>Dungeon Crawl을 다시 굴리지 않습니다.</small>
            </article>
          )}
          {room && viewRoomId !== 'entrance' && (
            <article className="crawl-room" key={room.id}>
              <span className="eyebrow">
                {room.kind === 'special' ? 'SPECIAL ROOM' : 'ROOM'} ·{' '}
          ...
          <div className="crawl-step crawl-next">
            {state.phase === 'room' && (
              <>
            ...
            {state.phase === 'ready' && (
              <>
                ...
                <Button onClick={next}>다음 방 · Dungeon Crawl 2d20</Button>
              </>
            )}
            {state.phase === 'entrance' && (
              <Button onClick={next}>첫 방으로 · Dungeon Crawl 2d20</Button>
            )}
          </div>
        </>
      )}
