...
export interface TrustedSearchTitle {
  text: string;
  origin:
    | 'explicit-title'
    | 'ui-name'
...
export function indexTrustedReferenceTitles(
  ...
  const index = new Map<string, TrustedSearchTitle>();
  for (const entry of registry.entries) {
    ...
    if (trusted) index.set(entry.id, trusted);
  }
...
export function trustedReferenceSearchTitle(
  registry: ReferenceRegistry,
