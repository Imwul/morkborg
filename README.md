...
한국어 UI로 사용하는 로컬 무작위 생성기와 캠페인 기록장입니다. 제목과 내용을 반복해서 굴리고 원하는 후보를 선택하여, Campaign 안에 여러 Character·Monster·Dungeon을 모으고, Dungeon마다 독립적인 Room과 기록을 보관합니다. 고유명사와 원문 생성표의 결과는 원래 표기를 유지합니다.
...
- 방 순서를 바꿔도 UUID로 연결된 배치는 유지됩니다. 방 삭제 시 몬스터 배치는 수량·메모를 유지한 채 Dungeon-only로 이동합니다. 던전 복제는 새 방 ID에 배치를 연결하고 몬스터 정의는 공유합니다.
